# Code for America Fit Map

A single, shareable web page that shows the **Code for America job description exactly as posted** — and lets a reader hover any highlighted phrase (it visibly becomes clickable) and click it to see the **specific, real experience** that backs it.
...
- The real Code for America posting: intro, **About the role**, **In this position you will**, **About you** — styled to feel like the live job page (Code for America logo, colors, and typefaces).
...
1. **Create a repo and push these files to its root:**
   ```bash
   cd code-for-america-brainmap
   git init
   git add index.html styles.css app.js data.json cfa-logo.svg README.md build_data.py
   git commit -m "Code for America fit map"
   git branch -M main
...
```
code-for-america-brainmap/
  index.html         # page structure (no content)
  styles.css         # Code for America brand styling + responsive
  ...
  cfa-logo.svg       # Code for America logo (white, for the indigo top bar)
